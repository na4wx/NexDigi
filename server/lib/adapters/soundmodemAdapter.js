...
class SoundModemAdapter extends EventEmitter {
  ...
  _connectAGW() {
    ...
    this.socket.on('error', (e) => { this.emit('error', e); this._scheduleReconnect(); });
    this.socket.on('close', () => { this._connected = false; this.emit('close'); this._scheduleReconnect(); });
  }
  ...
  _connectKissTcp() {
    ...
    this.socket.on('data', (d) => this.emit('data', d));
    this.socket.on('error', (e) => { this.emit('error', e); console.warn('SoundModemAdapter error', e.message); this._scheduleReconnect(); });
    this.socket.on('close', () => { this._connected = false; this.emit('close'); console.log('SoundModemAdapter socket closed'); this._scheduleReconnect(); });
  }
  ...
  close() {
    this._closing = true;
    if (this.socket) try { this.socket.end(); } catch(e){}
